...
RTSPSO::RTSPSO( std::shared_ptr<dx12lib::Device> device, bool enableLighting, bool enableDecal )
: m_Device( device )
, m_DirtyFlags( DF_All )
...
, m_EnableDecal( enableDecal )
{
    ...
    pipelineStateStream.VS                    = CD3DX12_SHADER_BYTECODE( RayCastShaderBlob.Get() );
    pipelineStateStream.PS                    = CD3DX12_SHADER_BYTECODE( MissShaderBlob.Get() );
...
inline void RTSPSO::BindTexture( CommandList& commandList, uint32_t offset, const std::shared_ptr<Texture>& texture )
{
    if ( texture )
    {
...
void RTSPSO::Apply( CommandList& commandList )
{
